...
class NavigateForwardButton(NavigationButton):
    def __init__(self, browsing_context: BrowsingContext, parent=None):
        ...
        browsing_context.web_page_history_can_go_forward.connect(self.__on_web_page_history_can_go_forward)

        # We fire off a signal to go forward in the page history when the button is clicked
        self.clicked.connect(browsing_context.web_page_history_go_forward.emit)

    @Slot(bool)
    def __on_web_page_history_can_go_forward(self, can_go_forward: bool) -> None:
